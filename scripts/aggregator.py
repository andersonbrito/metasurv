...
if __name__ == '__main__':
    ...
    # input = path + 'cases-brazil-states_reformatted.tsv'
    # unit = 'month'
    ...
    def unit_coverter(df):
        ...
        df['time_variable'] = df.index.map(get_newunit) # create new column 'epiweek', mapping 'dates'
        df = df.groupby(['time_variable'], as_index=True).sum() # group dates from same epiweek, sum up counts
        return df.transpose()
